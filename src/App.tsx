...
function PoesiaBox({ poesia }: { poesia: any }) {
  ...
  return (
    <div
      className="w-full border rounded-lg p-5 shadow mb-6 bg-white transition-all duration-300 ease-in-out"
      tabIndex={0}
      onKeyDown={e => {
      ...
      aria-controls={`analisi-${poesia.id}`}
      role="button"
      title="Premi Invio o Spazio per aprire/chiudere"
    >
      {/* Titolo e testo poesia */}
      <div
        className="cursor-pointer flex justify-between items-start select-none"
      ...
      >
        <div className="flex-1 pr-4">
          ...
          <p
            className={`text-gray-900 text-base transition-all duration-300 ${
              aperta ? '' : 'line-clamp-2'
            }`}
          >
            {poesia.content}
        ...
        <span
          className="text-indigo-600 font-semibold text-sm ml-3 select-none"
          aria-label={aperta ? 'Chiudi analisi' : 'Apri analisi'}
        >
          {aperta ? '▲ Chiudi' : '▼ Apri'}
        </span>
      </div>
      ...
      {/* Analisi */}
      <div
        id={`analisi-${poesia.id}`}
        className={`mt-6 space-y-6 max-h-0 overflow-hidden transition-[max-height] duration-500 ease-in-out ${
          aperta ? 'max-h-[1000px]' : ''
        }`}
        aria-hidden={!aperta}
      >
        <section className="bg-indigo-50 p-4 rounded shadow-inner border border-indigo-200">
          <h3 className="font-bold text-indigo-800 mb-2 border-b border-indigo-300 pb-1">
            Analisi Letteraria
          </h3>
          {poesia.analisi_letteraria ? (
            <>
              <p>
                ...
                <span className="text-indigo-700">{poesia.analisi_letteraria.stile_letterario || 'N/A'}</span>
              </p>
              <p>
                <strong>Temi:</strong>
              </p>
              ...
              <p>
                ...
                <span className="text-indigo-700">{poesia.analisi_letteraria.struttura || 'N/A'}</span>
              </p>
              <p>
                <strong>Riferimenti culturali:</strong>{' '}
                <span className="text-indigo-700">{poesia.analisi_letteraria.riferimenti_culturali || 'N/A'}</span>
              </p>
            </>
          ) : (
          ...
          {poesia.analisi_psicologica ? (
            <>
              ...
              <p>
                ...
                <span className="text-green-700">{poesia.analisi_psicologica.stato_interno || 'N/A'}</span>
              </p>
              <p>
                <strong>Visione del mondo:</strong>{' '}
                <span className="text-green-700">{poesia.analisi_psicologica.visione_del_mondo || 'N/A'}</span>
              </p>
            </>
          ) : (
...
export default function App() {
  ...
  return (
    <main
      className="max-w-lg sm:max-w-2xl mx-auto p-4 sm:p-6 bg-gray-50 min-h-screen"
      role="main"
      aria-label="Vetrina poesie analizzate"
    >
      <h1
        className="text-2xl sm:text-3xl font-bold mb-4 text-center text-green-700 tracking-wide mx-auto block"
        tabIndex={0}
      >
      ...
      <div className="mb-6">
        <label htmlFor="search" className="sr-only">
          Cerca poesie per titolo, autore o testo
        </label>
        <input
          id="search"
          type="search"
      ...
      {poesieFiltrate.length > 0 ? (
        poesieFiltrate.map(poesia => <PoesiaBox key={poesia.id} poesia={poesia} />)
      ) : (
        !loading && <p className="text-center text-gray-400 mt-10">Nessuna poesia trovata.</p>
      )}
    </main>
  )
}
