...
function PoesiaBox({ poesia }: { poesia: any }) {
  ...
  return (
    <div className="w-full border rounded-lg p-6 shadow-lg mb-6 bg-white transition-all hover:shadow-xl">
      ...
      >
        <div className="flex-1 pr-4">
          <h2 className="text-xl font-extrabold text-indigo-700">{poesia.title || 'Senza titolo'}</h2>
          <p className="text-sm italic text-gray-500 mb-3">{poesia.author_name || 'Anonimo'}</p>
          <p className={`text-gray-900 text-base leading-relaxed ${aperta ? '' : 'line-clamp-3'}`}>
            {poesia.content}
          </p>
        </div>
        <span className="text-indigo-600 font-semibold text-sm ml-4 select-none self-start mt-1">
          {aperta ? '▲ Chiudi' : '▼ Apri'}
        </span>
      </div>
      ...
      {aperta && (
        <div className="mt-8 space-y-6">
          <section className="bg-indigo-50 p-5 rounded shadow-inner border border-indigo-300">
            <h3 className="font-bold text-indigo-800 mb-3 border-b border-indigo-400 pb-2 text-lg">
              Analisi Letteraria
            </h3>
          ...
          <section className="bg-green-50 p-5 rounded shadow-inner border border-green-300">
            <h3 className="font-bold text-green-800 mb-3 border-b border-green-400 pb-2 text-lg">
              Analisi Psicologica
            </h3>
...
export default function App() {
  ...
  return (
    <main className="max-w-lg sm:max-w-3xl mx-auto p-6 bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-extrabold mb-6 text-center text-green-700 tracking-wide">
